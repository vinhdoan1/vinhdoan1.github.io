"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var http_1 = require('@angular/http');
require('rxjs/add/operator/toPromise');
var TripService = (function () {
    function TripService(http) {
        this.http = http;
        this.tripsUrl = 'app/trips'; // URL to web api
    }
    TripService.prototype.getTrips = function () {
        return this.http.get(this.tripsUrl)
            .toPromise()
            .then(function (response) { return response.json().data; })
            .catch(this.handleError);
    };
    TripService.prototype.getTrip = function (id) {
        return this.getTrips()
            .then(function (trips) { return trips.filter(function (trips) { return trips.id === id; })[0]; });
    };
    TripService.prototype.save = function (trip) {
        if (trip.id) {
            return this.put(trip);
        }
        return this.post(trip);
    };
    TripService.prototype.delete = function (trip) {
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        var url = this.tripsUrl + "/" + trip.id;
        return this.http
            .delete(url, headers)
            .toPromise()
            .catch(this.handleError);
    };
    // Add new City
    TripService.prototype.post = function (trip) {
        var headers = new http_1.Headers({
            'Content-Type': 'application/json'
        });
        return this.http
            .post(this.tripsUrl, JSON.stringify(trip), { headers: headers })
            .toPromise()
            .then(function (res) { return res.json().data; })
            .catch(this.handleError);
    };
    // Update existing City
    TripService.prototype.put = function (trip) {
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        var url = this.tripsUrl + "/" + trip.id;
        return this.http
            .put(url, JSON.stringify(trip), { headers: headers })
            .toPromise()
            .then(function () { return trip; })
            .catch(this.handleError);
    };
    TripService.prototype.handleError = function (error) {
        console.error('An error occurred', error);
        return Promise.reject(error.message || error);
    };
    TripService = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [http_1.Http])
    ], TripService);
    return TripService;
}());
exports.TripService = TripService;
/*
Copyright 2016 Google Inc. All Rights Reserved.
Use of this source code is governed by an MIT-style license that
can be found in the LICENSE file at http://angular.io/license
*/ 
//# sourceMappingURL=trip.service.js.map